/*
  # Core Database Schema for African Diaspora Relationship Platform

  1. New Tables
    - `profiles` - User profile information with cultural data
    - `cultural_backgrounds` - Detailed cultural heritage information
    - `personality_assessments` - Big Five personality test results
    - `cultural_quiz_results` - Cultural knowledge quiz scores
...
DROP TABLE IF EXISTS cultural_quiz_results CASCADE;
DROP TABLE IF EXISTS personality_assessments CASCADE;
DROP TABLE IF EXISTS cultural_backgrounds CASCADE;
...
CREATE INDEX idx_cultural_backgrounds_tribe ON cultural_backgrounds (primary_tribe);
CREATE INDEX idx_cultural_backgrounds_languages ON cultural_backgrounds USING GIN (languages_spoken);
...
CREATE INDEX idx_events_date_location ON cultural_events (event_date, location_coordinates);

-- Full-text search indexes
CREATE INDEX idx_profiles_search ON profiles USING GIN (to_tsvector('english', first_name || ' ' || last_name || ' ' || COALESCE(bio, '')));
CREATE INDEX idx_events_search ON cultural_events USING GIN (to_tsvector('english', title || ' ' || description));
